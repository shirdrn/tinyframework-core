...
public abstract class FileTinyJobRunner extends TinyJobRunner<TinyTask> 
	...
	public void configure() {
		...
		String waitingPath = jobConf.getContext().get("core.job.files.waiting.dir");
		...
		String completedPath = jobConf.getContext().get("core.job.files.completed.dir");
		...
		suffix = jobConf.getContext().get("core.job.files.suffix");
		LOG.info("File suffix name;suffix=" + suffix);
		// configure file charset
		String encoding = jobConf.getContext().get("core.job.files.charset");
		if(encoding!=null) {
			charSet = encoding;
