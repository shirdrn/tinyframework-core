...
public class SinglePoolTinyExecutorManager extends TinyExecutorManager<SinglePoolReadable> implements SinglePoolReadable {
	...
	public void setReadableContext(ReadableContext readableContext) {
		...
		this.corePoolSize = readableContext.getInt("core.job.executor.pool.corePoolSize", 1);
		this.maximumPoolSize = readableContext.getInt("core.job.executor.pool.maximumPoolSize", 1);
		this.keepAliveTime = readableContext.getInt("core.job.executor.pool.keepAliveTime", 1200);
		this.workQueueSize = readableContext.getInt("core.job.executor.pool.workQueueSize", 1);
		this.checkIdleWorkerInterval = readableContext.getInt("core.job.executor.pool.checkIdleWorkerInterval", 1);
		...
		String threadFactoryClassName = readableContext.get(
				"core.job.executor.thread.factory.class", 
